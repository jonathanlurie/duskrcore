...
class AdobeMetadataInterpolator {
  ...
  interpolate(){
    ...
    settingAttributeNames.forEach(attr => {
      ...
      let splineInterpolator = new splines.MonotonicCubicSpline(xs, ys);
    ...
    function interpolateCurve(color=''){
      ...
      for(let i=0; i<maxNbPoint; i++){
        ...
        let splineInterpolatorCurveX = new splines.MonotonicCubicSpline(xs, curveXs);
        let splineInterpolatorCurveY = new splines.MonotonicCubicSpline(xs, curveYs);
